...
test("runs a tool call against the real service", () =>
  run(
    [
      {
        type: "tool-call",
    ...
    Effect.gen(function*() {
      ...
      expect(response.text).toBe("Added it.")
      ...
      expect(listed[0]?.body).toBe("2 litres")
    })
  ))

test("hands a missing note back to the model instead of failing", () =>
  run(
    ...
    Effect.gen(function*() {
      ...
      expect(result?.name).toBe("get_note")
    })
  ))

test("rejects tool arguments that do not match the schema", () =>
  run(
